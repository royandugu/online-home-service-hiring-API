...
const validatePhoneOtp=async (req,res)=>{
    const {adminOtp,phoneNumber}=req.body;

    if(!adminOtp) throw new BadRequestError("The OTP is not present");
    if(adminOtp.length!=6) throw new BadRequestError("Invalid OTP format");
...
const registerAdmin=async (req,res)=>{
    ...
    if(!email) {
        ...
        return res.status(StatusCodes.CREATED).json({admin:result,message:"Admin created"});        
    }
    else{
...
const loginAdmin=async (req,res)=> {
    ...
    try {
      ...
      const admin = await AdminModel.findOne({ phoneNumber, password });
...
module.exports={sendPhoneOtp,validatePhoneOtp,registerAdmin,loginAdmin};
